...
config = ConfigParser()
config.read("/home/aiultrasys/workarea/softwares/workables/NayapayDatawarehouse_ETL/EndtoEndDataPipeline/conf.ini")
...
def main():
    ...
    if args.infraenv == "FullLoad":
        ...
        for i in range(4):
            for j in range(12):
                for k in range(31):
                    ...
                    strToInsertData = "ALTER TABLE creditbook_sourcelayer.analytics_details ADD PARTITION (cryear='"+yearList[i]+"',crmonth='"+monthList[j]+"',crday='"+dayList[k]+"') LOCATION '/creditBookDWH/analytics/cryear="+yearList[i]+"/crmonth="+monthList[j]+"/crday="+dayList[k]+"'"
                    #print(strToInsertData)
                    sessionHive.sql(strToInsertData)

        #### Here, we have data for these 4 years only and we are not catering incremental load data. So for time being we are taking 2024 too with all its months, this approach is not right for incremnetal, 
        ...
        ## IN incremental load we apply same logic, we DROP PARTITION for the same day, each time our cron job will ran and we again ADD partition of same day.
        ## First cron of the day will remove all data from yesterday and insert all day of yesterday again so that we do not miss any data point. 
